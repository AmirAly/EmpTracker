empTracker.controller("LoginController", function ($scope, $rootScope, $state, $timeout, API, $window, $ionicLoading, $cordovaNetwork, $http, $cordovaToast, LocalStorage) {
    $scope.frmmLogin = {};
    ...
    $scope.$on('$ionicView.enter', function () {
        ...
        if (!angular.equals(LocalStorage.getObject('UserLocalObject'), {}) && LocalStorage.getObject('UserLocalObject').UserName != "") { // not empty, then do login
            ...
            $scope.afterLoginError = false;
            $scope.userObj = LocalStorage.getObject('UserLocalObject');
            $timeout(function () {
                // do login
                var req = {
                    method: 'POST',
                                ...
                                if (_res.data.code = 200) {
                                    ...
                                    API.execute(req, true).then(function (_res) {
                                        ...
                                        if (_res.data.code = 200) {
                                            ...
                                            if ($rootScope.userSettings.TimeAttendanceSettings.EnableChallengeQuestions == true) {
                                                $rootScope.wakeupChallange();
                                            }
                                            $ionicLoading.hide();
                                            // if user is Employee
                                            LocalStorage.setObject('UserLocalObject', $scope.userObj);
